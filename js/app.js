...
let clear = (id) => {
	if (id === 'ce') {
		let localStrToArr = display.value.split(''); // 
		localStrToArr.pop();
		let localArrToStr = localStrToArr.toString();
		let localDeleteCommas = localArrToStr.replace(/,/g,"");
		display.value = localDeleteCommas;
	} else if (id === 'c') {
		display.value = '0';
		MemoryNewNumber = true;
		MemoryCurrentNumber = 0;
